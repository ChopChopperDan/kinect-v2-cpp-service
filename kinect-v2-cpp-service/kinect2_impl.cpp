...
RR_SHARED_PTR<RobotRaconteur::RRArray<uint64_t > > Kinect2_impl::getTrackedBodyIDs()
{
	boost::lock_guard<boost::mutex> guard(mtx_);

	RR_SHARED_PTR<RobotRaconteur::RRArray<uint64_t > > ids = RobotRaconteur::AttachRRArrayCopy<uint64_t >(reinterpret_cast<uint64_t *>(&this->tracked_body_ids), 6);
